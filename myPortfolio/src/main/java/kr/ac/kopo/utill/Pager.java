...
public class Pager {
	...
	public String getPagination() {
		...
		html ="<ul>";
		html += "<li><a href='?page=1'>처음</a></l i>";
		html += "<li><a href='?page="+ (dbPage < 2 ? 1: dbPage - 5) +"'>이전</a></li>";
		
		//반복문을 사용하여 페이징 블럭 처리
		for(int i= startPage; i <= endPage; i++) {
			//삼항연산지를 사용해 현재 접속한 page를 확인하여 클래스 부여(css 처리)
			html +="<li class='"+ (i == dbPage ? "current-page" : "")+"'><a href='?page="+i+"'>"+i+"</a></li>";	
		}
		
		html += "<li><a href='?page="+ (dbPage + 5 >= maxPage ? maxPage : dbPage + 5 ) + "'>다음</a></li>";
		html += "<li><a href='?page="+maxPage+"'>끝</a></li>";
		html +="</ul>";
		return html;
	}
